...
const getSynonymsFromClaude = async (word) => {
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Please provide 4 synonyms for the word "${word}" that would work well in most contexts. Respond with just the synonyms separated by commas, nothing else.`
        }
      ]
    });

    console.log('Got response from Claude:', message.content);
    const synonyms = message.content.split(',').map(s => s.trim());
    return synonyms;
  } catch (error) {
...
function App() {
  ...
  const handleTextChange = async (e) => {
    ...
    await processInputRealTime(e.target.value);
  };

  const processInputRealTime = async (currentText) => {
    ...
    console.log('Redundant words found:', redundant);
    ...
    for (const word of redundant) {
      ...
      if (synonymsDB[word]) {
        console.log('Found in local DB:', word);
        newSuggestions[word] = synonymsDB[word];
      } else {
        console.log('Not in local DB, trying Claude for:', word);
        const claudeSynonyms = await getSynonymsFromClaude(word);
        if (claudeSynonyms) {
          console.log('Got Claude synonyms:', claudeSynonyms);
          newSuggestions[word] = claudeSynonyms;
        }
      }
    }
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        setText(text);
        await processInputRealTime(text);
      } catch (error) {
